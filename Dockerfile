...
RUN apt-get update && apt-get install -y procps unzip openjdk-7-jre-headless mysql-client

ENV SONARQUBE_VERSION 4.1.2
...
RUN cd /opt/sonar/extensions/plugins && \
	curl -sLo sonar-sigmm-plugin-1.0.1.jar http://repository.codehaus.org/org/codehaus/sonar-plugins/sonar-sigmm-plugin/1.0.1/sonar-sigmm-plugin-1.0.1.jar
